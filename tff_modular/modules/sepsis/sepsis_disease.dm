...
/datum/disease/sepsis/stage_act(seconds_per_tick, times_fired)
	...
	if(has_antibiotics())
		seconds_with_spaceacillin += seconds_per_tick
	...
	switch(stage)
		if(1)
			if(SPT_PROB(5, seconds_per_tick))
				...
				carrier.adjust_coretemperature(5, max_temp = BODYTEMP_HEAT_DAMAGE_LIMIT)
		...
		if(3)
			...
			if(SPT_PROB(5, seconds_per_tick))
				...
				organs += carrier.get_organs_for_zone(BODY_ZONE_CHEST)
				organs += carrier.get_organs_for_zone(BODY_ZONE_HEAD)
		...
		if(5)
			...
			if(SPT_PROB(15, seconds_per_tick))
				carrier.adjust_coretemperature(4, max_temp = BODYTEMP_HEAT_DAMAGE_LIMIT + 20)
			...
			if(SPT_PROB(2, seconds_per_tick))
				to_chat(carrier, span_notice("You're struggling to breathe."))
				need_mob_update += carrier.adjustOxyLoss(10, updating_health = FALSE)
			if(SPT_PROB(10, seconds_per_tick))
