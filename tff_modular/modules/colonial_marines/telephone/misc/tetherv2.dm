...
/datum/transmitter_tether/New(atom/tether_parent, atom/tether_target, max_dist = 7, tether_name, icon, icon_file, can_be_blocked = FALSE)
	if(!ismovable(tether_parent) || !istype(tether_target) || !tether_target.loc)
		qdel(src)
		return
	...
	cur_dist = max_dist
	var/datum/beam/beam = tether_target.Beam(tether_parent, src.icon_name, src.icon_file, emissive = FALSE, beam_type = /obj/effect/ebeam/custom_tether, layer = GIB_LAYER)
	tether_beam = beam
...
/datum/transmitter_tether/Destroy(force)
	. = ..()
	UnregisterSignal(tether_parent, list(COMSIG_MOVABLE_PRE_MOVE, COMSIG_MOVABLE_MOVED, COMSIG_QDELETING))
...
/datum/transmitter_tether/proc/check_tether(atom/source, new_loc)
	SIGNAL_HANDLER
	...
	var/atom/movable/anchor = (source == tether_target ? tether_parent : tether_target)
...
/datum/transmitter_tether/proc/check_block(atom/movable/source, atom/movable/anchor, new_loc, silent = FALSE)
	var/atom/blocker
...
/datum/transmitter_tether/proc/check_snap()
	SIGNAL_HANDLER

	if(QDELETED(src))
		return

	var/atom/atom_target = tether_parent
...
/datum/transmitter_tether/proc/snap()
	SIGNAL_HANDLER
	...
	SEND_SIGNAL(tether_parent, COMSIG_ATOM_TETHER_SNAPPED)
	SEND_SIGNAL(tether_target, COMSIG_ATOM_TETHER_SNAPPED)
	qdel(src)

/datum/transmitter_tether/proc/on_delete()
	SIGNAL_HANDLER
	qdel(src)
